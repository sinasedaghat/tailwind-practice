...
<template>
  <!-- header -->
  <header class="relative font-sans antialiased z-50">
  <!-- <header class="relative font-nunito antialiased z-50"> -->
    <div class="fixed top-0 left-0 right-0 flex flex-nowrap flex-row gap-9 p-5">
      <!-- bg-black -->
      <!-- logo section -->
      <div class="basis-1/12 flex items-center flex-no-shrink text-white">
        <svg fill="none" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1180 320" class="a-icon--logo-text" data-new=""
          aria-hidden="true" data-v-cbc994d7="">
          <g fill="currentColor" data-v-cbc994d7="">
            <path
      ...
      <div class="basis-6/12">
        <nav>
          <ul class="list-none flex flex-row flex-nowrap gap-3 text-white">
            <li
              v-for="item in navItem"
              :key="item.id"
              class="basis-auto no-underline capitalize text-lg font-medium py-1 px-3"
            >
              {{ item.title }}
              <!-- <i class="mdi mdi-chevron-dow">mdi-chevron-down</i> -->
            </li>
          </ul>
        </nav>
      </div>
      ...
      <div class="basis-5/12">
        <nav>
          <ul class="list-none flex flex-nowrap flex-row gap-3 justify-end text-white">
            <li
              v-for="item in buttonItem"
              :key="item.id"
              class="basis-auto no-underline capitalize text-lg font-medium py-1 px-3"
              :class="{'border border-white hover:bg-white hover:text-black hover:cursor-pointer': item.type == 'border'}"
              >
              {{ item.text }}
              <!-- <i class="mdi mdi-chevron-dow">mdi-chevron-down</i> -->
            </li>
