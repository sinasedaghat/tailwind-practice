...
<template>
  <div class="pt-28 pb-16 px-6 flex flex-no-shrink text-white bg-black">
    <!-- logo -->
    <div class="basis-2/6">
      <div class="w-32 flex items-center flex-no-shrink text-white">
    ...
    <div 
      ...
      :key="links.id"
      class="basis-1/6 flex-col text-lg"
    >
      <!-- title -->
      <span class="font-semibold">{{ links.title }}</span>
      <!-- link -->
      <ul class="list-none flex flex-col gap-1">
        <li
          v-for="link in links.links"
          :key="link.id"
          class="no-underline hover:underline capitalize font-extralight"
        >
          <a :href="link.href" :target="link.icon ? '_blank' : '_self'">
            {{ link.text }}
